#include "../include/pid_position_follow.h"
...
RobotCtrl::RobotCtrl()
{
    ...
    ROS_INFO_STREAM("\033[1;32m----> PID_position_follow  started.\033[0m");
    ...
    odom_sub_ = nh.subscribe("/Odometry", 10, &RobotCtrl::Odom_Callback, this);
    ...
    base_yaw_sub_ = nh.subscribe("/base_yaw_vel",5,&RobotCtrl::Base_yawCallback,this);
...
void RobotCtrl::Odom_Callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    robot_pose_.pose.position.x = msg->pose.pose.position.x;
    robot_pose_.pose.position.y = msg->pose.pose.position.y;
}

void RobotCtrl::Base_yawCallback(const std_msgs::Float64 &msg)
{
    set_yaw_speed_ = msg.data;
}
...
void RobotCtrl::Plan(const ros::TimerEvent &event)
{
    ...
    if (1)
    {
        if (plan_)
        {
            ...
            // printf("------>tf: %0.2f, %0.2f", robot_pose.pose.position.x, robot_pose.pose.position.y);
            // printf("------>odom: %0.2f, %0.2f", robot_pose_.pose.position.x, robot_pose_.pose.position.y);
        ...
        else
        {
            ...
            nav_msgs::Path non_path;
            non_path.header.frame_id = global_frame_;
            non_path.header.stamp = ros::Time::now();
            local_path_pub_.publish(non_path);
